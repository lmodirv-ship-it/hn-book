      ...
      for (const book of verifiedBooks) {
        ...
        try {
          ...
          try {
            ...
            if (dlResp.ok) {
              const respContentType = dlResp.headers.get("content-type") || "";
              if (respContentType.includes("image")) {
                fileExt = respContentType.includes("png") ? "png" : "jpg";
                contentType = respContentType;
              } else if (respContentType.includes("svg")) {
                fileExt = "svg";
                contentType = "image/svg+xml";
              }

              const fileData = await dlResp.arrayBuffer();
              const path = `${folder}/${timestamp}-${fileExt === "pdf" ? "file" : "asset"}.${fileExt}`;
              const bucket = fileExt === "pdf" ? "book-files" : "book-images";

              const { error: upErr } = await supabase.storage
                .from(bucket)
                .upload(path, new Uint8Array(fileData), { contentType, upsert: true });
              if (!upErr) {
          ...
          if (coverUrl) {
            try {
              ...
              if (coverResp.ok) {
                ...
                const coverPath = `${folder}/${timestamp}-cover.jpg`;
          ...
          // Insert product — reference_code auto-generated by DB trigger
          ...
          if (!insertErr && product) {
            const refCode = product.reference_code || product.id.substring(0, 8);
            ...
            if (fileUrl && storagePath) {
              ...
              await supabase.from("product_files").insert({
                ...
                file_name: `${refCode}.${fileExt}`,
                storage_path: storagePath,
                public_url: fileUrl,
                is_primary: true,
              });
            }
            imported.push({ ...book, _imported: true, _product_id: product.id, _code: refCode });
          } else {

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Auth check — admin only (skip for cron)
    ...
    const { data: noPdf } = await svc
      .from("products").select("id, name, is_active")
      .or("pdf_url.is.null,pdf_url.eq.").limit(200);
      ...
      if (mode === "fix") {
        for (const book of noPdf) {
          if (book.is_active !== false) {
            ...
            actions.push({ type: "no_pdf", bookId: book.id, bookName: book.name, action: "تعطيل الكتاب (بدون PDF)", result: error ? "failed" : "fixed", detail: error?.message || "تم تعطيله" });
          } else {
            actions.push({ type: "no_pdf", bookId: book.id, bookName: book.name, action: "كتاب معطل مسبقاً", result: "skipped", detail: "معطل مسبقاً" });
          }
        }
      }
    }
    ...
    const { data: noCover } = await svc
      .from("products").select("id, name, image")
      .or("image.is.null,image.eq.,image.eq./placeholder.svg").limit(200);
    ...
    if (noCover && noCover.length > 0) {
      ...
      if (mode === "fix") {
        for (const book of noCover) {
          const { error } = await svc.from("products").update({ image: "/placeholder.svg" }).eq("id", book.id);
          actions.push({ type: "no_cover", bookId: book.id, bookName: book.name, action: "تعيين غلاف مؤقت", result: error ? "failed" : "fixed", detail: error?.message || "تم تعيين placeholder" });
        }
      }
    }
    ...
    const { data: failedJobs } = await svc
      .from("upload_jobs").select("id, file_name, result")
      .eq("status", "error").limit(50);

    if (failedJobs && failedJobs.length > 0) {
      issues.push({ type: "failed_jobs", count: failedJobs.length, description: "مهام رفع فاشلة" });
      if (mode === "fix") {
        const ids = failedJobs.map(j => j.id);
        const { error } = await svc.from("upload_jobs").update({ status: "pending" }).in("id", ids);
        actions.push({ type: "failed_jobs", action: `إعادة ${ids.length} مهمة فاشلة`, result: error ? "failed" : "fixed", detail: error?.message || `تمت إعادة ${ids.length} مهمة` });
      }
    }
    ...
    const { data: noCategory } = await svc
      ...
      .or("category.is.null,category.eq.").limit(200);
    ...
    if (noCategory && noCategory.length > 0) {
      ...
      if (mode === "fix") {
        for (const book of noCategory) {
          const { error } = await svc.from("products").update({ category: "كتب" }).eq("id", book.id);
          actions.push({ type: "no_category", bookId: book.id, bookName: book.name, action: "تعيين تصنيف افتراضي (كتب)", result: error ? "failed" : "fixed", detail: error?.message || "تم التعيين" });
        }
      }
    }
    ...
    const { data: zeroPriceBooks } = await svc
      .from("products").select("id, name, page_count, category")
      .eq("price", 0).not("page_count", "is", null).gt("page_count", 0).limit(200);
    ...
    if (zeroPriceBooks && zeroPriceBooks.length > 0) {
      ...
      if (mode === "fix") {
        const { data: rules } = await svc.from("pricing_rules").select("min_pages,max_pages,price_per_page,priority,country,paper_type").eq("is_active", true);
        const { data: baseSetting } = await svc.from("pricing_settings").select("value").eq("key", "base_price_per_page").single();
        const basePrice = (baseSetting?.value as any)?.value ?? 1;

        for (const book of zeroPriceBooks) {
          const pc = book.page_count as number;
          const matching = (rules || []).filter((r: any) => pc >= r.min_pages && pc <= r.max_pages).sort((a: any, b: any) => b.priority - a.priority);
          ...
          actions.push({ type: "zero_price", bookId: book.id, bookName: book.name, action: `تسعير تلقائي: ${price} د.م`, result: error ? "failed" : "fixed", detail: error?.message || `${price} د.م` });
        }
      }
    }
    ...
    const { data: stuckJobs } = await svc
      .from("upload_jobs").select("id, file_name")
      .eq("status", "processing").lt("updated_at", tenMinAgo).limit(50);
    ...
    if (stuckJobs && stuckJobs.length > 0) {
      ...
      if (mode === "fix") {
        ...
        const { error } = await svc.from("upload_jobs").update({ status: "pending" }).in("id", ids);
        actions.push({ type: "stuck_jobs", action: `إعادة ${ids.length} مهمة عالقة`, result: error ? "failed" : "fixed", detail: error?.message || `تمت إعادة ${ids.length} مهمة` });
      }
    }

...
const SectionRow = ({
  title, icon: Icon, items, isEditable, onLocked,
}: {
  title: string;
  icon: typeof Sparkles;
  items: Asset[];
  isEditable: (a: Asset) => boolean;
  onLocked: (a: Asset) => void;
}) => {
  ...
  return (
    <section className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {items.map((a) => (
          <PremiumCard key={a.id} asset={a} editable={isEditable(a)} onLocked={onLocked} />
        ))}
      </div>
    </section>
  );
...
const PremiumCard = ({
  asset, editable, onLocked,
}: { asset: Asset; editable: boolean; onLocked: (a: Asset) => void }) => {
  const meta = ASSET_TYPE_META[asset.asset_type];
  const navigate = useNavigate();
  const location = useLocation();
  const inStudio = location.pathname.startsWith("/studio");
  const editHref = `/studio/editor/${asset.id}?from=${inStudio ? "studio" : "templates"}`;

  const handlePrimary = (e?: React.MouseEvent | React.KeyboardEvent) => {
    e?.preventDefault();
    if (editable) navigate(editHref);
    else onLocked(asset);
  };
  ...
  return (
    <div
      ...
      onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") handlePrimary(e); }}
    ...
    >
      ...
      <div className="relative rounded-xl overflow-hidden bg-card border border-border/60 group-hover:border-primary/50 transition-all duration-300">
        <div className="relative aspect-[4/5] overflow-hidden bg-muted">
          ...
          {editable ? (
            <div className="absolute top-3 left-3 px-2 py-1 rounded-md bg-primary/90 text-primary-foreground text-[10px] font-semibold backdrop-blur inline-flex items-center gap-1">
              <Sparkles className="w-3 h-3" /> قابل للتعديل
            </div>
          ) : (
            <div className="absolute top-3 left-3 px-2 py-1 rounded-md bg-muted/80 text-muted-foreground text-[10px] font-semibold backdrop-blur inline-flex items-center gap-1 border border-border/60">
              <Lock className="w-3 h-3" /> تصميم ثابت
            </div>
          )}
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-background via-background/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4 gap-2">
            {editable ? (
              <Button
                size="sm"
                className="w-full gap-1.5 shadow-lg shadow-primary/30"
                onClick={(e) => { e.stopPropagation(); navigate(editHref); }}
              >
                <Edit3 className="w-3.5 h-3.5" /> ابدأ التصميم
              </Button>
            ) : (
              <Button
                size="sm"
                variant="outline"
                className="w-full gap-1.5 border-primary/40 text-foreground hover:bg-primary/10"
                onClick={(e) => { e.stopPropagation(); onLocked(asset); }}
              >
                <Printer className="w-3.5 h-3.5" /> اطلب الآن
              </Button>
            )}
          </div>
        </div>
...
/** Modal shown when the user taps a non-editable (fixed) template. */
const LockedTemplateDialog = ({
  asset, onClose,
}: { asset: Asset | null; onClose: () => void }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const open = !!asset;
  const orderHref = asset ? getRouteFor(asset.asset_type, asset.id) : "#";
  const browseHref = location.pathname.startsWith("/studio") ? "/studio/templates?editable=1" : "/templates?editable=1";
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent dir="rtl" className="max-w-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Lock className="w-4 h-4 text-primary" /> هذا التصميم غير قابل للتعديل
          </DialogTitle>
          <DialogDescription>
            يمكنك طلبه كما هو، أو اختيار قالب آخر يدعم التعديل.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter className="flex-col sm:flex-row sm:justify-end gap-2">
          <Button variant="outline" onClick={() => { onClose(); navigate(browseHref); }} className="gap-1.5">
            <Sparkles className="w-3.5 h-3.5" /> اختيار قالب آخر
          </Button>
          <Button onClick={() => { onClose(); navigate(orderHref); }} className="gap-1.5">
            <Printer className="w-3.5 h-3.5" /> اطلب كما هو
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>

...
import {
  ...
  Maximize2, Minimize2, Sun, Moon, X
} from "lucide-react";
...
const ReaderHeader = ({
  ...
  isFullscreen, showSidebar, showSearch, isBookmarked, isDarkTheme,
  onToggleSidebar, onToggleSearch, onToggleFullscreen, onToggleBookmark, onToggleTheme,
}: ReaderHeaderProps) => {
  ...
  return (
    <header className={`flex-shrink-0 h-12 ${bg} backdrop-blur border-b ${border} flex items-center justify-between px-3 z-50`}>
      <div className="flex items-center gap-1.5">
        ...
        <button onClick={onToggleSearch} className={`p-1.5 rounded-lg ${btnHover} transition-colors ${showSearch ? (isDarkTheme ? "bg-white/10 text-white" : "bg-gray-200 text-gray-800") : ""}`}>
          <Search className="w-4 h-4" />
        </button>
      </div>

      <div className="flex items-center gap-1 min-w-0 max-w-[40%]">
        <p className={`text-xs font-medium ${textMain} truncate`}>{bookName}</p>
        <span className={`text-[10px] ${textSub} flex-shrink-0 hidden sm:inline`}>
          {category} · {referenceCode}
        </span>
      </div>

      <div className="flex items-center gap-0.5">
        <button onClick={onToggleTheme} className={`p-1.5 rounded-lg ${btnHover} transition-colors`} title={isDarkTheme ? "وضع فاتح" : "وضع داكن"}>

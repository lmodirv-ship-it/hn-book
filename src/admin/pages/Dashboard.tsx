...
const RecentOrderRow = ({ name, product, amount, status }: { name: string; product: string; amount: string; status: string }) => (
  <div className="flex items-center justify-between py-3 border-b border-border/50 last:border-0">
    ...
    <div className="text-left">
      ...
      <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${
        status === "مكتمل" ? "bg-green-400/10 text-green-400" :
        status === "معلق" ? "bg-yellow-400/10 text-yellow-400" :
...
const recentOrders = [
  { name: "أحمد محمد", product: "Ultimate eBook Collection", amount: "$29.99", status: "مكتمل" },
  { name: "سارة علي", product: "Pro Design Templates", amount: "$49.99", status: "مكتمل" },
  { name: "خالد يوسف", product: "AI Mastery Course", amount: "$39.99", status: "معلق" },
  { name: "فاطمة حسن", product: "Business Growth Bundle", amount: "$59.99", status: "قيد التنفيذ" },
  { name: "عمر أمين", product: "Language Learning Pack", amount: "$19.99", status: "مكتمل" },
];
...
const AdminDashboard = () => {
  ...
  const stats = [
    ...
    { icon: ShoppingCart, label: "الطلبات اليوم", value: "47", change: "+8%", changePositive: true },
    { icon: DollarSign, label: "الأرباح اليوم", value: "$2,847", change: "+23%", changePositive: true },
    { icon: Users, label: "العملاء الجدد", value: "156", change: "+5%", changePositive: true },
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                ...
                <th className="text-right py-3 px-2 text-xs text-muted-foreground font-medium">المبيعات</th>
            ...
            <tbody>
              {topProducts.map((p, i) => (
                <tr key={p.id} className="border-b border-border/50 last:border-0 hover:bg-secondary/30 transition-colors">
                  ...
                  <td className="py-3 px-2 text-muted-foreground">{Math.floor(50 + i * 30)}</td>
                  <td className="py-3 px-2">
